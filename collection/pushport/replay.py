...
def parse_arrival(record: ET.Element, namespace_dictionary: dict[str, str], location: collection.pushport.classes.Location):
    time = collection.pushport.classes.Time(
        ...
        actualRemoved=record.get("atRemoved"),
    ...
    print((time.location.locationTIPLOC, time.location.trainStatus.rid))
    print(time.actualTime)
    print(location.trainStatus.startDate)
...
def parse_departure(record: ET.Element, namespace_dictionary: dict[str, str], location: collection.pushport.classes.Location):
    time = collection.pushport.classes.Time(
        location=location,
        estimatedTime=record.get("et"),
        workingEstimatedTime=record.get("wet"),
...
def parse_passing(record: ET.Element, namespace_dictionary: dict[str, str], location: collection.pushport.classes.Location):
    time = collection.pushport.classes.Time(
        location=location,
        estimatedTime=record.get("et"),
        workingEstimatedTime=record.get("wet"),
...
def parse_location(record: ET.Element, namespace_dictionary: dict[str, str], train_status: collection.pushport.classes.TrainStatus):
    ...
    for arrival in arrival_records: parse_arrival(arrival, namespace_dictionary, location)

    departure_records = record.findall("namespace:dep", {"namespace": namespace_dictionary.get("fc")})
    for departure in departure_records: parse_departure(departure, namespace_dictionary, location)

    passing_records = record.findall("namespace:pass", {"namespace": namespace_dictionary.get("fc")})
    for passing in passing_records: parse_passing(passing, namespace_dictionary, location)
...
def parse_train_status(record: ET.Element, namespace_dictionary: dict[str, str], date: datetime.date, update: collection.pushport.classes.Update):
    ...
    for location in location_records: parse_location(location, namespace_dictionary, train_status)

def parse_update(record: ET.Element, namespace_dictionary: dict[str, str], date: datetime.date):
    ...
    for train_status in train_status_records: parse_train_status(train_status, namespace_dictionary, date, update)
...
def replay_file(path, date: datetime.date):
    for chunk_data in chunk_file(path):
        ...
        for update in update_records: parse_update(update, namespace_dictionary, date)
